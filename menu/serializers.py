...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def get_items(self, obj):
        items = obj.items.filter(is_active=True).prefetch_related(
            "branches", "ingredients__ingredient", "ingredients__ingredient__unit"
        )

        branch_id = self.context.get("branch_id")

        if branch_id:
            items = items.filter(branches__id=branch_id)

        return MenuItemSerializer(items.distinct(), many=True).data
