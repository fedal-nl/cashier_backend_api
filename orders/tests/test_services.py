...
class OrderServiceTest(TestCase):
    def setUp(self):
        ...
        self.ingredient = Ingredient.objects.create(
            name_ar="جبنة", price=2, unit=self.unit
        )
    ...
    def test_update_order_status_writes_status_log(self):
        ...
        update_order_status(
            order=order, status=Order.OrderStatus.PREPARING, user=self.user
        )

        order.refresh_from_db()
        log = order.logs.filter(event_type=OrderLog.EventType.STATUS_UPDATED).get()
    ...
    def test_update_order_status_sets_delivery_company(self):
        ...
        self.assertEqual(order.delivery_company, delivery_company)

        self.assertEqual(order.status, Order.OrderStatus.PICKED_UP)
    ...
    def test_create_order_with_items_with_modifications(self):
        ...
        self.assertEqual(order.items.count(), 1)
        self.assertEqual(
            order.total_price, 14
        )  # 10 for the burger + 2*2 for the added cheese
        item = order.items.first()
        assert item is not None
    ...
    def test_create_order_with_multiple_items_and_modifications(self):
        another_menu_item = MenuItem.objects.create(
            name_ar="بطاطس", price=5, category=self.category, quantity=1, unit=self.unit
        )
        ...
        self.assertEqual(
            order.total_price, 22
        )  # (10 + 2) for the burger with cheese + (5*2) for the fries
    ...
    def test_create_order_with_removed_ingredient_modification(self):
        ...
        self.assertEqual(
            order.total_price, 8
        )  # 10 for the burger - 2 for the removed cheese
        item = order.items.first()
        assert item is not None
        self.assertEqual(item.modifications.count(), 1)
    ...
    def test_create_order_without_ingredient_in_modification(self):
        ...
        )  # the modification should be ignored since it doesn't have an ingredient
        item = order.items.first()
        assert item is not None
        self.assertEqual(item.modifications.count(), 0)
