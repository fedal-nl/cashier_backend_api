...
class DailyReportViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.second_new_customer = Customer.objects.create(
            name="Second New Customer",
            email="second-new@test.com"
        )
        ...
        self.assertEqual(
            previous_order.created_at.date().isoformat(),
            "2026-05-31"
        )
    ...
    def test_daily_report_returns_rows_for_date_range(self):
        ...
        self.assertEqual(
            first_day["total_orders"],
            3
        )
        self.assertEqual(
            first_day["orders_by_status"]["created"],
            2
        )
        self.assertEqual(
            first_day["orders_by_status"]["completed"],
            1
        )
        self.assertEqual(
            first_day["orders_by_status"]["picked_up"],
            0
        )
        self.assertEqual(
            first_day["total_new_customers_ordered"],
            1
        )
        self.assertEqual(
            first_day["total_existing_customers_ordered"],
            1
        )
        self.assertEqual(
            first_day["total_revenue"],
            "35.00"
        )
        ...
        self.assertEqual(
            second_day["total_orders"],
            2
        )
        self.assertEqual(
            second_day["orders_by_status"]["picked_up"],
            1
        )
        self.assertEqual(
            second_day["orders_by_status"]["delivered"],
            1
        )
        self.assertEqual(
            second_day["total_new_customers_ordered"],
            1
        )
        self.assertEqual(
            second_day["total_existing_customers_ordered"],
            1
        )
        self.assertEqual(
            second_day["total_revenue"],
            "22.00"
        )
    ...
    def test_daily_report_can_be_filtered_by_status(self):
        ...
        self.assertEqual(
            data[0]["orders_by_status"]["completed"],
            1
        )

        self.assertEqual(
            data[0]["orders_by_status"]["created"],
            0
        )
